    ...
    if sed_result:
        ...
        try:
            ...
            if "i" in flags and "g" in flags:
                text = re.sub(fr"{repl}", u"{}".format(repl_with), to_fix, flags=re.I).strip()
            elif "i" in flags:
                text = re.sub(fr"{repl}", u"{}".format(repl_with), to_fix, count=1, flags=re.I).strip()
            elif "g" in flags:
                text = re.sub(fr"{repl}", u"{}".format(repl_with), to_fix).strip()
            else:
                text = re.sub(fr"{repl}", u"{}".format(repl_with), to_fix, count=1).strip()
        except sre_err as e:
            await bot.send_message(message.chat.id, "ERROR: {e}")
            return
